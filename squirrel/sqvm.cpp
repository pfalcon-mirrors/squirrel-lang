...
{
	_lasterror=SQString::Create(_ss(this),err);
}
	...
	try{
		...
		//	printf("\n[%d] %s %d %d %d %d\n",ci->_ip-ci->_iv->_vals,g_InstrDesc[_i_.op].name,arg0,arg1,arg2,arg3);
			...
			case _OP_NEWTABLE:
				...
				continue;
			case _OP_NEWARRAY:
				TARGET=SQArray::Create(_ss(this),0); _array(TARGET)->Reserve(arg1);
				continue;
...
SQObjectPtr SQVM::CallNative(SQObjectPtr &nclosure,int nargs,int stackbase,bool tailcall)
{
	...
	if(ret<0)sqraise_str_error(_ss(this),_stringval(_lasterror));
	SQObjectPtr retval;
	if(ret!=0){ retval=TOP(); }
